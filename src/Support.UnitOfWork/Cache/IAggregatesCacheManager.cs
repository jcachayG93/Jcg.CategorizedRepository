...
namespace Support.UnitOfWork.Cache
{
    ...
    {
        /// <summary>
        /// Gets the data from the cache, if the key was not read before, gets the data from the database and adds it to the cache.
        /// </summary>
        ...
        /// <summary>
        /// Upserts the data in the local cache. If the key was not read before will read it from the database and add it to the cache.
        ...
        /// that was received when the items were read from the database. If the item was inserted then the ETag will be a blank string.
        /// </summary>
        IEnumerable<UpsertedItem<TAggregateDatabaseModel>> UpsertedItems { get; }
    }
}
